...
| Bài học lỗi ĐÃ XÁC ĐỊNH CHẮC CHẮN nguyên nhân, dùng chung nhiều project | Simplenote (tag project hiện tại + tag kỹ thuật để project khác cũng tìm được) — xem mục "Ghi bài học lỗi vào Simplenote" |
...
- **Bài học lỗi ĐÃ XÁC ĐỊNH CHẮC CHẮN nguyên nhân → Simplenote (cross-project, KHÁC AgentMemory):**
  ...
  - **Trước khi tạo mới, LUÔN tìm trước:** Tìm trên Simplenote theo cả tag kỹ thuật (loại lỗi/framework/tầng hệ thống liên quan — xem cách chọn tag bên dưới) LẪN tag project hiện tại, vì note phù hợp có thể do project khác tạo trước đó. Chỉ lọc riêng theo tag project hiện tại khi tìm theo tag kỹ thuật ra quá nhiều kết quả không liên quan.
  ...
  - **Tag:** Note luôn phải được gắn tag = tên project hiện tại (đúng giá trị biến `project` theo "QUY ƯỚC ĐỊNH DANH" ở đầu file) để biết note phát sinh từ đâu. NGOÀI RA, vì mục đích chính của kho này là để **project KHÁC** cũng tìm lại được, phải gắn THÊM các tag mô tả bản chất lỗi (không chỉ dừng ở tag project) để tra cứu xuyên project dễ dàng, ví dụ:
    - Ngôn ngữ/framework/runtime liên quan (vd: `nodejs`, `react`, `postgresql`, `docker`).
    - Loại lỗi/chủ đề kỹ thuật (vd: `race-condition`, `null-pointer`, `cors`, `memory-leak`, `auth`, `migration`).
    - Tầng hệ thống liên quan nếu rõ ràng (vd: `database`, `api`, `frontend`, `ci-cd`).
    - Chỉ gắn tag nào THỰC SỰ mô tả đúng bản chất lỗi, không gắn tràn lan cho "chắc ăn". Trước khi đặt tag mới, ưu tiên tìm/liệt kê tag đã có sẵn trong Simplenote và dùng lại nếu trùng ý nghĩa, tránh tạo tag trùng nghĩa nhưng khác cách viết (vd: vừa có `nodejs` vừa có `node-js`).
    - Nếu chắc chắn bài học này cũng từng áp dụng cho project khác đã biết, có thể gắn thêm tag project đó — không tự đoán tag của project chưa từng làm.
...
## Role & Context
- **Project:** Quote Widget — mobile app hiển thị quote/nhắc nhở trên Home Screen widget Android
- **Tech:** Flutter (SDK ^3.33), Dart ^3.13.1, Hive (local DB), home_widget (widget bridge)
- **Platform:** Android only, minSdk=24, app ID: com.quotewidget.quotewidget
- **Agent role:** Flutter developer — fix bugs, add features, code review
- **No state management library** — useState/setState only
- **No formal routing** — Navigator 1.0 imperative

## Initial Setup & Navigation
`context.md`/`working.md`/`operating_rules.md` đã được đọc tự động ở "SESSION START" — KHÔNG đọc lại. Project structure:
- `lib/models/` — Hive data models (Collection, Item, WidgetConfig)
- `lib/services/` — Business logic (StorageService, WidgetService, BackupService...)
- `lib/screens/` — UI screens
- `lib/widgets/` — Reusable widgets
- `android/.../widget/QuoteWidgetProvider.kt` — Native Android widget (330 lines)

## Critical Rules
1. **Widget data flow:** Flutter writes via `HomeWidget.saveWidgetData()` → `HomeWidgetPreferences` file. Kotlin reads from same file. NEVER use `SharedPreferences.getInstance()` for widget data (wrong file).
2. **SharedPreferences files matter:** `HomeWidgetPreferences` for widget data, `FlutterSharedPreferences` for supplementary (is_pro, configured_widget_ids). Default SharedPreferences is NOT used.
3. **No INTERNET permission** — app is offline-first by design.
4. **Ponytail FULL by default** — minimize code, avoid over-engineering.
5. **All 44 tests must pass** before any commit.
6. **`flutter analyze` must show 0 errors, 0 warnings** before commit.
7. **Dead code:** `widget_config_screen.dart` is unreachable (never imported). Don't add features to it.

## Workflow
- **Bug fixing:** Read `context.md` for architecture → check `operating_rules.md` for conventions → fix → `flutter test` → `flutter analyze` → report
- **Feature add:** Update `working.md` after completion. Test thoroughly before reporting.
- **Widget changes:** Always verify SharedPreferences file alignment (HomeWidgetPreferences vs FlutterSharedPreferences).

## Git & Kiểm thử
- **Test command:** `flutter test` (44 tests)
- **Lint command:** `flutter analyze`
- **No git commits yet** (project in development)

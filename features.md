...
> Cập nhật lần cuối: **2026-09-07** (Final Hardening Batch `fix(final-hardening)` — P0-1..P2-3 đủ 6 bug, **170/170 tests**, analyze 0 issues; trước đó: Feature Close `c3b6f73` pushed, cleanup prefs `f54623f`/`1fff5bc`, release-engineering `70deddb` + fix CI `e1a0060`). Local ahead 4 (hardening + docs + 2 release-eng) chờ credential để push.
...
- **2026-09-07 (final hardening, `.plan/prompt_final_hardening_batch.md`):** commit local `fix(final-hardening)`. P0-1 snapshot content-only (không lưu/restore WidgetConfig — hết phantom; 2 test). P1-1 reorder bị disable + guard khi Search/Favorites active (3 widget test). P1-2 daily pin lưu `daily_item_id`, same-day ổn định khi delete/reorder, xóa item pin → chọn lại giữ `daily_date`; Kotlin advance ngày clear id stale + onDeleted/onRestored dọn key (3 test + extend 1). P1-3 setup rollback khi bước sau `createWidgetConfig` fail + snackbar lỗi (1 widget test). P2-1 duplicate copy author (1 test). P2-2 dead `widget_config_screen`/`widget_preview` — verify đã xóa (0 ref, analyze sạch). P2-3 AddWidgetGuide probe support thay auto-pin khi mở màn (1 widget test). **170/170 tests, analyze 0 issues.** Chi tiết: `result10.txt`.
...
- **Final Hardening Batch (2026-09-07):** `fix(final-hardening)` + review-fix `4dc6def` local — snapshot content-only (P0-1), reorder filter-safe (P1-1), `daily_item_id` (P1-2), setup rollback (P1-3), duplicate author (P2-1), dead-code verify (P2-2), guide no auto-pin (P2-3), crash-guard daily index âm (review). 172/172 tests. Chi tiết: `result10.txt`.
